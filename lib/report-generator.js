...
function generateReports(results) {
  ...
  if (!fs.existsSync(pemDir)) fs.mkdirSync(pemDir, { recursive: true });

  const jsonData = JSON.stringify(buildJson(results), null, 2);
  const htmlData = buildHtml(results);

  fs.writeFileSync(path.join(pemDir, '.myCert.json'), jsonData, 'utf8');
  fs.writeFileSync(path.join(pemDir, '.myCert-report.html'), htmlData, 'utf8');
  console.log(`[output] pem/.myCert-report.html、pem/.myCert.json`);
}
