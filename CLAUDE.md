...
**輸出檔案**（每次執行均會覆寫）：
- `output/report.html` — HTML 樹狀報表，可展開/收合節點
- `output/result.json` — 機器可讀的 JSON 結構，含完整樹狀資料
- `pem/<Root CA>/` — 有效 UCA 憑證（`.crt`，可在 Windows 直接開啟或匯入）
- `pem/expire/<Root CA>/` — 已過期 UCA 憑證（`.crt`）
...
| `lib/config-loader.js` | 讀取 `config.json`，驗證並回傳 `rootCAWhitelist` 與 `ccadb`（`diffStoreLimit`、`diffDisplayLimit`）；匯出 `loadConfig` |
...
| `lib/report-generator.js` | 產生 `output/report.html`（Root CA 為樹根的互動報表）與 `output/result.json`；匯出 `generateReports` |
...
- `downloadHistory[]`：每次下載的時間戳記（GMT+8）、總筆數、與前次的 diff（`addedCount`、`added[]`）
- 使用快取時仍會顯示上次下載的 diff，確保不遺漏新增項目
- `diffStoreLimit` / `diffDisplayLimit` 可在 `config.json` 的 `ccadb` 區塊調整
...
- 有效憑證（`validTo >= now`）→ `pem/<Root CA 名稱>/<UCA 名稱>.crt`
- 已過期憑證 → `pem/expire/<Root CA 名稱>/<UCA 名稱>.crt`
...
- `/security-check` — 對所有 `*.js` 進行靜態安全分析（XSS、注入、路徑穿越、原型污染）及 Node.js v18 現代化審查。
- `/verify` — 確認所有 `.js` 能正常執行，並檢查關鍵 v18 API（`fetch`、`AbortSignal.timeout`、`fs.promises` 等）是否正確使用。
- `/update-docs` — 依程式碼實際狀態同步更新 `README.md`、`CLAUDE.md` 及 `docs/operator.html`。
